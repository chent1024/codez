...
const LINUX_DEEP_LINK_DESKTOP_FILE = "codez.desktop";
const LINUX_DEEP_LINK_MIME_TYPE = "x-scheme-handler/codez";
// 归属标记：用于识别用户级 codez.desktop 是否由本应用写入（历史所有版本都带这行 Comment）。
const LINUX_DESKTOP_ENTRY_OWNERSHIP_MARKER = "Comment=CodeZ Desktop App";
...
function resolveLinuxDeepLinkCommand(params: {
  ...
  return {
    ...
    // AppImage 的 codez:// 回调会由 xdg-open 按 .desktop Exec 二次启动。
...
function createLinuxDeepLinkDesktopEntry(params: {
  ...
  const iconName = params.iconName ?? "codez";
  const command = {
...
function removeOwnedUserDesktopEntry(
  ...
  if (!isOwnedDesktopEntry(desktopFilePath)) {
    logger.warn("[deep-link] Linux 用户级 codez.desktop 非本应用写入，保留不清理", {
      desktopFilePath,
    });
    return;
  }
  try {
    ...
    logger.info("[deep-link] 已清理遗留的用户级 codez.desktop，恢复系统级条目", {
      desktopFilePath,
    });
  } catch (error) {
    logger.warn("[deep-link] 清理遗留用户级 codez.desktop 失败", { desktopFilePath, error });
  }
}
...
export function registerLinuxDeepLinkProtocol(options: RegisterLinuxDeepLinkProtocolOptions): void {
  ...
  // 用户级 codez.desktop 在 XDG
  // 解析中永远优先于系统级同名条目。rpm/deb 安装后，旧 AppImage 写入的用户级条目会把
  // /usr/share/applications/codez.desktop 持续遮蔽，快捷方式和 codez:// deep link 一直
  ...
  // 用户手写的自定义 codez.desktop（无归属标记）不受影响，保留不清理。
