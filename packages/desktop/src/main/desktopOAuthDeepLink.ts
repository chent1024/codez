...
export function resolveExternalWorkspaceOpenDialogCopy(
  ...
  return {
    ...
    title: "Open external CodeZ link?",
    message: "Open this folder in CodeZ?",
    detail: (path) =>
...
export function handleDeepLink(
  ...
  if (isWorkspaceOpenUrl(parsedUrl)) {
    ...
    const targetWindow = options.resolveApplicationWindow
      ? options.resolveApplicationWindow()
      : (BrowserWindow.getFocusedWindow() ?? BrowserWindow.getAllWindows()[0] ?? null);
    // codez://workspace/open 来自浏览器/IM 等外部应用，不能等同于用户在
    // CodeZ 内部选择目录；确认必须发生在 statSync 之前，避免项目配置被静默信任。
    if (
